...
# Loading data from an RData set
```{r}
setwd("C:/Users/Sergio/Dropbox/UPC/FIB/Analisis de datos y explotacion de la informacion (ADEI)/Proyecto/")
#setwd("C:/Users/usuario/Documents/ADEI/FIB-ADEI-Big-Data-Analysis-master")
...
# contact
sel<-which(df$contact=="unknown");sel
df$contact[sel]<-NA
summary(df$contact)
df$contact<-factor(df$contact)

# month
sel<-which(df$month=="unknown");sel
df$month[sel]<-NA
summary(df$month)
df$month<-factor(df$month)

# day_of_week
sel<-which(df$day_of_week=="unknown");sel
df$day_of_week[sel]<-NA
summary(df$day_of_week)
df$day_of_week<-factor(df$day_of_week)

# campaign
sel<-which(df$campaign=="unknown");sel
df$campaign[sel]<-NA
summary(df$campaign)
df$campaign<-factor(df$campaign)

# pdays
sel<-which(df$pdays=="unknown");sel
df$pdays[sel]<-NA
summary(df$pdays)
df$pdays<-factor(df$pdays)

# previous
sel<-which(df$previous=="unknown");sel
df$previous[sel]<-NA
summary(df$previous)
df$previous<-factor(df$previous)

# poutcome
sel<-which(df$poutcome=="unknown");sel
df$poutcome[sel]<-NA
summary(df$poutcome)
df$poutcome<-factor(df$poutcome)

# emp.var.rate
sel<-which(df$emp.var.rate=="unknown");sel
df$emp.var.rate[sel]<-NA
summary(df$emp.var.rate)
df$emp.var.rate<-factor(df$emp.var.rate)

# cons.price.idx
sel<-which(df$cons.price.idx=="unknown");sel
df$cons.price.idx[sel]<-NA
summary(df$cons.price.idx)
df$cons.price.idx<-factor(df$cons.price.idx)

# cons.conf.idx
sel<-which(df$cons.conf.idx=="unknown");sel
df$cons.conf.idx[sel]<-NA
summary(df$cons.conf.idx)
df$cons.conf.idx<-factor(df$cons.conf.idx)

# euribor3m
sel<-which(df$euribor3m=="unknown");sel
df$euribor3m[sel]<-NA
summary(df$euribor3m)
df$euribor3m<-factor(df$euribor3m)

# nr.employed
sel<-which(df$nr.employed=="unknown");sel
df$nr.employed[sel]<-NA
summary(df$nr.employed)
df$nr.employed<-factor(df$nr.employed)
