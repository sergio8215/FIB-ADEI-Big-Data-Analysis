...
aux2<-imputeMCA(df[,aux],ncp=10)#preguntar hay diferencia entre imputar antes o despues de unir subcategorias(preferiblemente recategorizar despues de la imputación)
...
df[,aux]<-aux2$completeObs#Aceptamos las imputaciones realizadas
...
df$f.job <- 5
# 1 level - Admin 
aux<-which(df$job %in% c("admin."))
df$f.job[aux] <-1

# 2 level - Bussines 
aux<-which(df$job %in% c("entrepreneur", "management", "self-employed"))
df$f.job[aux] <-2

# 3 level - Not working
aux<-which(df$job %in% c("housemaid", "retired","unemployed","student"))
df$f.job[aux] <-3

# 4 level - Serv-Tech
aux<-which(df$job %in% c("services","technician"))
df$f.job[aux] <-4

# 5 level - Blue-Collar
aux<-which(df$job %in% c("blue-collar"))
df$f.job[aux] <-5
...
#no-target+numerica: Revisión de cada variable con summary y boxplots
  ...
  # Para el data analisis guardamos los outliers
  dqr[dqr$variable=='campaing','errors']<-20 #preguntar a sergio porqué 20???
  # Para el data analisis guardamos los errores
  dqr[dqr$variable=='campaing','outliers']<-0
  # Para los individuales
  dqri[aux,'outliers']<-dqri[aux,'outliers']+1

#Para pdays/previous/outcome podemos detectar errores si encontramos algún valor nonexistent ó 0 y que no sea 0 ó nonexistent respectivamente y además pdays=999

  aux<-which(df$pdays==999)
  aux2<-which(df$previous==0)
  aux3<-which(df$poutcome=='nonexistent')
  length(setdiff(aux3,aux2))#Cuantos en aux3 no están en aux2
  length(setdiff(aux2,aux3))
  length(setdiff(aux2,aux))
  length(setdiff(aux,aux2))
  #Vemos que hay pdays=999 que tienen previous>0 y poutcome!='nonexistent'
  summary(df[setdiff(aux,aux2),c('previous','poutcome')])#Miramos el perfil de esos individuos
  ...
  dqri[aux,'missings']<-dqri[aux,'missings']+1
	...
	aux3<-calcQ(aux2)
	abline(h=aux3[8],col="red",lwd=2) #Fijamos límite soft
	abline(h=aux3[9],col="magenta",lwd=2) #Fijamos límite extreme
  ...
  }#Aparecen muchas discordancias #preguntar si es necesario cosiderar errores para estas variables pues o bien son índices locales que varían según la zona o bien son errores en introducir el mes o el valor
  

	# emp.var.rate
  levels(factor(df$emp.var.rate))
  ...
  dqr[dqr$variable=='emp.var.rate','outliers']<-0
	...
	# cons.price.idx
  levels(factor(df$cons.price.idx))
  ...
  #Provisional para pruebas--------------- BORRAR
  #save.image("pro")
  #rm(list=ls())
  #load("pro")
  #--------------------------------------- FIN BORRAR
...
# Para pdays
aux<-quantile(df$pdays,seq(0,1,0.25),na.rm=TRUE)#Niveles por quartiles
aux<-factor(cut(df$pdays,breaks=aux,include.lowest=T))
table(aux)
tapply(df$pdays,aux,median)
aux2<-c(5,120,180,300,2100)#Niveles "naturales"
aux<-factor(cut(df$pdays,breaks=aux2,include.lowest=T))
table(aux)
tapply(df$pdays,aux,median)
df$f.pdays<-factor(cut(df$pdays,breaks=aux2,include.lowest=T))#Nos quedamos con los niveles naturales
levels(df$f.pdays)<-paste0("f.pdays-",levels(df$f.pdays))#Hacemos las etiquetas más informativas
summary(df$f.pdays)

# Para previous
aux<-quantile(df$previous,seq(0,1,0.25),na.rm=TRUE)#Niveles por quartiles
aux<-factor(cut(df$previous,breaks=aux,include.lowest=T))
table(aux)
tapply(df$previous,aux,median)
aux2<-c(5,120,180,300,2100)#Niveles "naturales"
aux<-factor(cut(df$previous,breaks=aux2,include.lowest=T))
table(aux)
tapply(df$previous,aux,median)
df$f.previous<-factor(cut(df$previous,breaks=aux2,include.lowest=T))#Nos quedamos con los niveles naturales
levels(df$f.previous)<-paste0("f.previous-",levels(df$f.previous))#Hacemos las etiquetas más informativas
