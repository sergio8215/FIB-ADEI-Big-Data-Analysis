...
df[,setdiff(vars_cat,no_imputadas)]<-aux2$completeObs[,setdiff(vars_cat,no_imputadas)] # Aceptamos las imputaciones realizadas, solo guardamos las variables que tenian NA's
...
levels(df$f.job)<-paste0("f.job.",levels(df$f.job)) # Hacemos las etiquetas más informativas
summary(df$f.job)
...
#Para pdays/previous/outcome creemos que estan relacionadas, por lo que podemos detectar errores si encontramos algún valor nonexistent ó 0 y que no sea 0 ó nonexistent respectivamente y además pdays=999
  ...
  summary(df[,vars_num])
  summary(vars_num_imp$completeObs)#Observamos que da valores razonados, solamente que debemos redondearlos en ambos casos ya que se trata de "número de contactos"
...
prop.table(table(df$foreign,SwissLabor$participation),1)
prop.table(table(df$foreign,SwissLabor$participation),2)
catdes(SwissLabor,1)
