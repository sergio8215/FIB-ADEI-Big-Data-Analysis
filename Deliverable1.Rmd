...
###Duration
Vemos que hay valores muy pequeños, incluso 0, también valores muy grandes. Miramos distribución en el histograma. De él se desprende que las llamadas son mayormente de aproximadamente 250 segundos, como ya anticipaba el summary.
...
###Comprobación de inconsistencias en los índices trimestrales/mensuales
  ...
  Aparecen muchas discordancias, ya que para cada individuo y para un mismo mes el valor debería ser el mismo y en este caso no lo son.
...
###Emp.var.rate,cons.price.idx, cons.conf.idx, euribor3m, nr.employed
  Necesitamos saber cómo se han obtenido estos datos para poder validarlos, como no tenemos esa información solo podemos comprobar los missings values.
...
Realizamos la imputación de las variables numéricas y comparamos los datos imputados con los originales.
...
Contrastamos también los boxplot para cada variable.
```{r}
  for(i in vars_num){
    par(mfrow=c(1,2))
...
Observamos que da valores razonados, solamente que debemos redondearlos en ambos casos ya que se trata de "número de contactos" de las variables 'previous' y 'campaign'. Igual que en  el caso anterior, solo se sobrescriben las variables imputadas en nuestro df. Contrastamos los summaries originales con los imputados.
...
```
###Campaign
  Como para esta variable la mayoría de los valores están entre 0 y 1, no se puede hacer la separación por cuartiles.
...
Campaing aun estando inversamente relacionada con duration, su correlación es muy pequeña pues no llega al 6%. Traducido al lenguaje natural podemos decir: "hay indicios de que cuantas más campañas ha participado el individuo más corta será la duración de la llamada".
