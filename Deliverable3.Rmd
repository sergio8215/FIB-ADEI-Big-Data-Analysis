...
Ahora probaremos seleccionando las variables a partir de la criba anterior: #preguntar diferencia summary y Anova, sabemos que el Anova nos dice para cada variable la probabilidad de que el modelo sea igual con o sin esta variable mediante el p-value, entonces que nos dice el summary de diferente? Res. Hay que usar los dos. El anova dice cuales variables iene los efectos netos relevantes.
. al principio de todo no usar anova porque dan lo mismo.
...
```{r}
# chunk 30
# hacer un vif antes del step, este chunk sobra.
m4<-step(m) #para reducir las variables que no son relevantes. no la apliquemos al primer cambio porque debemos pulir un poco el modelo primero. quitar variables correlacionadas etc, lueego de que no hay problemas de colinealidad tenemos las variabls significativas usamos el step que nos quitaras las cariables redundantes segun el criterio bayesiano.
```
...
par(mfrow=c(2,2))
plot(m5)
par(mfrow=c(1,1)) # la viad e eliminacion de las variablesredundantes que hemos seguido si se pone de acuerdo con este step.
# asi que si dice que ya esta bien, en esta fase l usamos como confirmacion. de las partes mas adelante no
```
...
Analizando los gráficos:   
  - Residual VS Fitted. En este gráfico muestra los residuos de los valores predecidos. Lo deseable es que los puntos estén uniformemente dispersos, para poderlo contrastar el gráfico esta provisto de una recta smoother que conviene que sea horizontal, y uniforme. En nuestro caso el resultado es aceptable.   # hay patron pero no se puede hacer nada con una culaidad tan pesima, es una hoja
...
Boxplot(cooks.distance(m9)) #preguntar nos sale el 3660 pero en el grafico anterior no sale, porque? y si hay que quitarlo al final, porque al final?# es influente porque nos hace meter variables explicativas. no lo quitamos no se sabe, cuando tengamos unacierta seguridad sobre las variables numericastransformaciones t favtore podemos plantearnos eso de quitarla o no. si lo hacemos al final de todo recalculamos el model t vemos que todas las ariales que sean significativas sean coeerentes quitando el influente
```
...
#preguntar que significa hacer interaccion entre factor y covariate (esta en el enunciado tercer titulo en negritas). se refiere a varible numerica explicativa(covariete) No usar month proque tien muchos niveles y seria un interaccion complicadaa
#preguntar se debe hacer asi? comparando el modelo con el step. Porque? cual cogemos usando el anova.
```{r}
# chunk 130
m30<-lm(log(duration)~(campaign+f.pdays+month)^2,data=df)#miramos que meses de verdad sn relevantes de la resta apara crear un factor que tenga menos niveles. season deben de como este no sirve, hayq ue ver la agrupacion correcta de month que nos perita capturar esta particularidad de month isn consumir tantos grados de libertad.
summary(m30)
m31<-step(m30,k=log(nrow(df)))
Anova(m31)
anova(m31,m30) #es una decsion nuestra que modelo usamos de estos dos modelos diferentes, se a cargado el month potque tiene muchos parametros.# Fisher test - Priority to BIC criteria
...
# mirar el influent y boxplot de la dist de cook
...
#preguntar se puede usar el step? no, son cosas distintas, el catdes te dce cuales sn relevantes por separads, mejor usar Anova
#preguntar, no es necesario Anova aqui? cual es el mas indicado summary o Anova-> Anova, aunque tambien se puede usar summary, igual que antes.
```
