...
#m9m<-lm(log(duration)~campaign+pdays,data=df[-3660,])
m9m<-lm(log(duration)~campaign+pdays,data=df[c(-3660,-4185,-2216),])
...
# chunk 120
# Y ~ A   Yes
m21<-lm(duration~1,data=df[])
summary(m21)
m22<-lm(duration~.,data=df[,c("duration","y",vars_cat)])
model.matrix(m22)
summary(m22)
Anova(m22)
anova(m21,m22) # Fisher test - Hauria de funcionar
```
